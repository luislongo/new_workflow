---
description: "Analisa o repositório, gera ADRs em meta/adr/ e atualiza CLAUDE.md com decisões arquiteturais"
model: opus
---
...
O `CLAUDE.md` é o ponto de entrada de contexto para o Claude Code — atualize-o com as decisões arquiteturais para que toda sessão inicie ciente das restrições ativas.

**Se `CLAUDE.md` já existir**, atualize apenas a seção `## Restrições e Decisões Arquiteturais` sem remover outras seções (como `## Documentação do Projeto` adicionada por `/generate-docs`).
...
> Execute `/generate-docs` para adicionar a seção de documentação do projeto.

## Restrições e Decisões Arquiteturais

Leia os ADRs em `meta/adr/` **antes de propor mudanças de stack, arquitetura ou padrões**.

| ADR | Título | Status | Categoria |
|-----|--------|--------|-----------|
| [ADR-0001](meta/adr/0001-...) | [Título] | Ativa | [Stack / Arquitetura / Dados / etc.] |
...
A tabela deve listar **todos os ADRs gerados**, com link relativo para o arquivo, título completo, status atual e categoria.
