...
Esta é uma aplicação de estudo com cinco telas que serão implementadas e depois mantidas por tarefas controladas. Precisamos definir os princípios arquiteturais que guiarão as decisões de design de código — não apenas a estrutura de pastas, mas como responsabilidades são separadas, como componentes crescem e como mudanças são contidas.
...
- As cinco telas compartilham a mesma entidade (`Project`) e as mesmas operações CRUD — há um domínio real, mesmo que simples
...
- O custo real de Clean Architecture para cinco telas com uma entidade é baixo: quatro use cases, um repositório em memória, uma interface de container
...
- As cinco telas compartilham a mesma entidade `Project`; sem uma camada de domínio, essa entidade vive em um local arbitrário
...
**Contras**: Curva de aprendizado não trivial com regras de importação próprias. Excessivo para cinco telas com uma entidade.
...
**2026-09-09**: Decisão inicial. Arquitetura implementada com `InMemoryProjectRepository`. Telas existem como stubs (`return null`) — serão implementadas no decorrer do experimento.
